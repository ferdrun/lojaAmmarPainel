...
class Users extends Model {
	...
	public function add($id_permission, 
	$email,
	$password,
	$name
	) {
		...
		if(!empty($email) && !empty($name) && !empty($password)  ) {
			$sql = "INSERT INTO users (id_permission, email, password, name) VALUES (:id_permission, :email, :password, :name)";
			$sql = $this->db->prepare($sql);
     ...
     public function edit($nome,
						  $email,
						  $permissions) {
		if(!empty($email) && !empty($nome) && !empty($permissions)) {
			print_r($nome);
			exit;
			$sql = "UPDATE users SET id_permission = :permission, email = :email, name = :nome WHERE id = :id";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':id_permission', $permissions);
			$sql->bindValue(':email', $email);
